'''
# summary table에 들어가는 code와 SQL과 결과를 분리한 것 -> summary를 하면 퍼포먼스도 좋아지고, 시각화도 좋는 등 여러 이슈 때매 분리해서 config는 분석가, 여기 코드는 엔지니어가 관리
# 여기 코드는 어떤 summary table를 빌드해주는 driver 같은 것
# 어떤 summary table 을 만드는가는 config에 있는 dic이 제공되어 있음
'''
